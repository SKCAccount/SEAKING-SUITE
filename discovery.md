...
- **2026-08-12, same session** — **Arc C added** to Kraken's §Major arcs: SPV entities. Larger deals get their own LLCs (true-JV 30/70-style with a founder management fee, or LP/GP with a promote waterfall; multiple investors per SPV), each with separate books, reporting, and contribution/distribution calculations — the payoff case for Arc A's multi-entity design. Decided: one entity-tagged Kraken (no per-SPV instances), per-SPV configurable distribution engine, investor portal v1 = capital account + deal performance + documents, K-1s as a staged CPA data pack. No fixed trigger (nice-to-have). Full context in the Kraken entry.
